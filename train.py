...
def train_and_evaluate(model, train_dataloader, dev_dataloader, optimizer, loss_fn, metrics, params, model_dir,
                       restore_file=None):
    ...
    for epoch in range(params.num_epochs):
        ...
        dev_metrics = evaluate(model, loss_fn, dev_dataloader, metrics, params)
        ...
        if is_best:
            ...
            # Save best dev metrics in a json file in the model directory
            best_json_path = os.path.join(model_dir, "metrics_dev_best_weights.json")
            utils.save_dict_to_json(dev_metrics, best_json_path)

        # Save latest dev metrics in a json file in the model directory
...
if __name__ == '__main__':
    ...
    train_and_evaluate(model, train_dl, dev_dl, optimizer, loss_fn, metrics, params, args.model_dir,
                       args.restore_file)
